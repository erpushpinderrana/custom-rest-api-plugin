...
/**
 * Restful Entity Reference formatter.
 *
 * @FieldFormatter(
 *   id = "restful_entity_reference_formatter",
 *   module = "custom_rest_api",
 *   label = @Translation("Restful Entity Reference Formatter"),
 *   field_types = {
 *     "entity_reference",
 *   },
 * )
 */
class RestfulEntityReferenceFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    if (count($items) > 0) {
      foreach ($items as $delta => $item) {
        ...
        // @todo Get the entity type and pass if it makes sense.
        // We need separate formatter for each entity so let it be Node for now.
    ...
    $elements['#key'] = 'custom_reference_key';
